...
if (x.strftime("%x"))>(g.strftime("%x")):
 print('\n\n')
 print("     "+'اهلا حبيبي . الاداه توقفت . الاداه بيه اشتراك اذا حاب \n			تشترك راسل ريفن : @MPMPK')
 print('\n\n')
 print(x)
...
if (x.strftime("%x"))==(g.strftime("%x")):
   ...
   if(x.strftime("%X"))>(g.strftime("%X")):
    print('\n\n')
    print("     "+'خلص اشتراك عليك مراجعة صانع اداة https://t.me/MPMPK☄️🗿')
    print('\n\n')
    print(x)
...
def dump_publik():
	with requests.Session() as ses:
		...
		try:
			...
			yu = f"""[bold yellow] 💀 Total ID 💀 :  \033[2;32m """+str(len(id))
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [ RAVEN ] ㅅ %s/%s ㅅ OK:%s ㅅ CP:%s ㅅ %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
